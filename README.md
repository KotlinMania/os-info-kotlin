...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`os_info`](https://crates.io/crates/os_info).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `os_info` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Detect host OS and version
...
Original work copyrighted by the upstream `os_info` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
