# Claude Code Project Instructions — os-info-kotlin

## Project Overview

This is **os-info-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`os_info`](https://crates.io/crates/os_info).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/os_info/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/os_info>
